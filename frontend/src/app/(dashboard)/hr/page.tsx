    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
        ...
        <StatCard label="Departments" value={departments.length} accentClass="bg-violet-50 text-violet-600"
          icon={<svg className="h-5 w-5 sm:h-6 sm:w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>}
        />
        <StatCard label="Avg Sentiment" value={sentiment ? sentiment.averageSentimentScore.toFixed(2) : '—'} accentClass="bg-emerald-50 text-emerald-600"
        ...
        />
        <StatCard label="Appraisals" value={sentiment?.count ?? '—'} accentClass="bg-amber-50 text-amber-600"
      ...
      <div className="grid lg:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6">
        <Card>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-gray-900 text-sm sm:text-base">Department Performance</h2>
              ...
              {departments.slice(0, 5).map((dept) => (
                <div key={dept.department}>
                  <div className="flex justify-between text-sm mb-1">
                    <span className="font-medium text-gray-700 truncate mr-2">{dept.department}</span>
                    <span className={`font-semibold shrink-0 ${getScoreColor(dept.averageFinalScore)}`}>{dept.averageFinalScore.toFixed(1)}</span>
                  </div>
        ...
        <Card>
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-gray-900 text-sm sm:text-base">Top Performers</h2>
            <Link href="/hr/analytics" className="text-xs text-indigo-600 hover:underline">View all →</Link>
          </div>
            ...
            <div className="space-y-3">
              {topPerformers.map((p, i) => (
                <div key={p._id} className="flex items-center gap-3">
                  <div className={`h-7 w-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${i === 0 ? 'bg-amber-100 text-amber-700' : 'bg-gray-100 text-gray-600'}`}>#{i + 1}</div>
      ...
      <Card>
        <h2 className="font-semibold text-gray-900 mb-4 text-sm sm:text-base">Quick Actions</h2>
