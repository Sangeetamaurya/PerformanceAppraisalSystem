...
function ResetPasswordModal({ modal, onClose, onConfirm }: ResetPasswordModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={modal.status === 'loading' ? undefined : onClose} />
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-5 sm:p-6 mx-4">
          ...
          <>
            ...
            <div className="bg-gray-50 rounded-xl p-4 mb-5">
              <p className="text-sm text-gray-700">
                ...
                <span className="text-gray-400 break-all">({modal.employee.email})</span>.
              </p>
              <p className="text-sm text-gray-500 mt-2">They will be required to change it on their next login.</p>
            </div>
            {modal.error && <ErrorAlert message={modal.error} className="mb-4" />}
            <div className="flex gap-3">
              <Button variant="secondary" className="flex-1" onClick={onClose}>Cancel</Button>
              <Button variant="danger" className="flex-1" onClick={onConfirm}>Reset Password</Button>
            </div>
          </>
        )}
        ...
        {modal.status === 'done' && modal.temporaryPassword && (
          <>
            ...
            <p className="text-sm text-gray-600 mb-3">New temporary password for <span className="font-semibold text-gray-800">{modal.employee.name}</span>:</p>
            <div className="flex items-center gap-2 bg-gray-900 rounded-xl px-4 py-3 mb-2">
              <code className="flex-1 text-emerald-400 font-mono text-sm sm:text-base tracking-widest break-all">{modal.temporaryPassword}</code>
              <button onClick={copyToClipboard} className="shrink-0 text-gray-400 hover:text-white transition-colors" title="Copy to clipboard">
                {copied ? (
...
export default function HREmployeesPage() {
  ...
  async function handleConfirmReset() {
    ...
    try {
      ...
      setResetModal(prev => prev ? { ...prev, status: 'confirm', error: extractApiError(err) } : null);
    }
  }
  ...
  const scoreBar = (value: number, max = 100) => (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
        <div className="h-full bg-indigo-400 rounded-full" style={{ width: `${(value / max) * 100}%` }} />
      </div>
  ...
  const starRating = (rating: number) => (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => (
        <svg key={s} className={`h-3.5 w-3.5 ${s <= rating ? 'text-amber-400' : 'text-gray-200'}`} fill="currentColor" viewBox="0 0 20 20">
  ...
  return (
    <div>
      <PageHeader title="Employees" subtitle={`${employees.length} total employees in the system`} />

      {resetSuccess && <SuccessAlert message={resetSuccess} className="mb-4" />}
      ...
      <Card padding="none">
        ...
        {isLoading ? (
          <div className="py-20 flex justify-center"><LoadingSpinner text="Loading employees..." /></div>
        ) : error ? (
                  ...
                  {filtered.map((emp) => (
                    <tr key={emp._id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-4 py-3">
                        <Badge className="bg-gray-100 text-gray-600 border-gray-200 font-mono">{emp.employeeId}</Badge>
                      </td>
                      <td className="px-4 py-3">
                        <div>
                      ...
                      <td className="px-4 py-3">{starRating(emp.peerRating)}</td>
                      <td className="px-4 py-3">
                        <Button variant="ghost" size="sm" onClick={() => openResetModal(emp)} className="text-amber-600 hover:bg-amber-50 hover:text-amber-700 whitespace-nowrap">
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <Badge className="bg-gray-100 text-gray-600 border-gray-200 font-mono text-xs">{emp.employeeId}</Badge>
                        <span className="text-xs text-gray-500">{emp.department}</span>
                      </div>
                    </div>
                    <Button variant="ghost" size="sm" onClick={() => openResetModal(emp)} className="text-amber-600 hover:bg-amber-50 hover:text-amber-700 shrink-0">
                      Reset
                    </Button>
                  </div>
                  <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                    <div>
                      <p className="text-xs text-gray-400 mb-1">KPI Score</p>
                      {scoreBar(emp.kpiScore)}
                    </div>
                    <div>
                      <p className="text-xs text-gray-400 mb-1">Attendance</p>
                      {scoreBar(emp.attendancePercentage)}
                    </div>
                    <div>
                      <p className="text-xs text-gray-400 mb-1">Sales %</p>
                      {scoreBar(emp.salesAchievementPercentage)}
                    </div>
                    <div>
                      <p className="text-xs text-gray-400 mb-1">Peer Rating</p>
                      {starRating(emp.peerRating)}
                    </div>
                  </div>
                </div>
              ))}
      ...
      {resetModal && (
        <ResetPasswordModal modal={resetModal} onClose={handleCloseModal} onConfirm={handleConfirmReset} />
      )}
    </div>
  );
}
