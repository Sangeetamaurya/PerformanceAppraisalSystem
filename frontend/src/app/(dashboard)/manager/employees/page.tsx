...
export default function ManagerEmployeesPage() {
  ...
  const starRating = (rating: number) => (
    <div className="flex gap-0.5">
      {[1,2,3,4,5].map((s) => (
        <svg key={s} className={`h-4 w-4 ${s <= rating ? 'text-amber-400' : 'text-gray-200'}`} fill="currentColor" viewBox="0 0 20 20">
                ...
                <tbody className="divide-y divide-gray-50">
                  {filtered.map((emp) => (
                    <tr key={emp._id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3"><Badge className="bg-gray-100 text-gray-700 border-gray-200">{emp.department}</Badge></td>
              ...
              {filtered.map((emp) => (
                <div key={emp._id} className="p-4">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="h-10 w-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 text-sm font-bold shrink-0">
                      {emp.name.charAt(0).toUpperCase()}
                    </div>
                    ...
                    <Badge className="bg-gray-100 text-gray-700 border-gray-200 shrink-0 text-xs">{emp.department}</Badge>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div className="bg-gray-50 rounded-lg px-3 py-2">
                      <p className="text-xs text-gray-400">KPI Score</p>
                      <p className="font-semibold text-gray-800">{emp.kpiScore}</p>
                    </div>
                    <div className="bg-gray-50 rounded-lg px-3 py-2">
                      <p className="text-xs text-gray-400">Attendance</p>
                      <p className="font-semibold text-gray-800">{emp.attendancePercentage}%</p>
                    </div>
                    <div className="bg-gray-50 rounded-lg px-3 py-2">
                      <p className="text-xs text-gray-400">Sales</p>
                      <p className="font-semibold text-gray-800">{emp.salesAchievementPercentage}%</p>
                    </div>
                    <div className="bg-gray-50 rounded-lg px-3 py-2">
                      <p className="text-xs text-gray-400">Peer Rating</p>
