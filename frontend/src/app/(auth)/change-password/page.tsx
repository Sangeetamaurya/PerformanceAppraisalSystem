...
export default function ChangePasswordPage() {
  ...
  // If already logged in and NOT first login, they shouldn't be here
  useEffect(() => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Update the user in context/sessionStorage — isFirstLogin is now false
      updateUser({ isFirstLogin: false });
      setSuccess(true);

      // Redirect to their dashboard after a short delay
      setTimeout(() => {
        router.push(ROLE_HOME[user?.role ?? 'Employee']);
      }, 1800);
    } catch (err) {
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gray-950 flex-col justify-between p-12">
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm leading-relaxed">
            Your account was created with a temporary password. Please set a new password
            to protect your data before accessing the system.
          </p>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-500 text-sm">
              Welcome, <span className="font-medium text-gray-700">{user?.name}</span>! You must change
          ...
          {/* Alerts */}
          {apiError && (
            <ErrorAlert message={apiError} onDismiss={() => setApiError(null)} className="mb-6" />
          )}
          {success && (
            <SuccessAlert
